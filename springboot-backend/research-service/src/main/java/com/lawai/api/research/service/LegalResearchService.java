...
public class LegalResearchService {
  ...
  public LegalResearchResponse run(String query, Consumer<ResearchStepDto> progressListener) {
    ...
    try {
      ...
      throw new LegalResearchException("error.research-source-failed", exception.getCause());
    }
    ...
    return new LegalResearchResponse(plan, steps, sourceResults, answer, i18n.get("research.disclaimer"), null);
  }
  ...
  private void emit(Consumer<ResearchStepDto> progressListener, ResearchStepType type, ResearchStepStatus status) {
    ...
    progressListener.accept(new ResearchStepDto(type, i18n.get(type.messageCode()), status));
  }
  ...
  private List<ResearchStepDto> buildCompletedSteps() {
    ...
    for (ResearchStepType type : ResearchStepType.values()) {
      steps.add(new ResearchStepDto(type, i18n.get(type.messageCode()), ResearchStepStatus.COMPLETED));
    }
