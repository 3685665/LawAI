...
Özet (kısa)
- Proje: Mono-repo — `backend` (Python API), `frontend` (Next.js), `springboot-backend` (Spring microservices).
...
- .github/copilot-instructions.md veya daha kapsamlı rol-temelli agent yönergeleri eklemek: `backend` / `frontend` / `springboot-backend` için ayrı kısımlar.
