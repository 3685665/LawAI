...
import { AlertCircle, CheckCircle2, Clock3, FolderOpen, LoaderCircle, Play, Plus, RefreshCw, Trash2 } from "lucide-react";
import {
...
function generateAutoJobName(form: BatchDocumentJobPayload) {
  const folder = folderNameFromPath(form.directoryPath);
  const schedule =
  ...
  const raw = suffix ? `${folder}-${schedule}-${suffix}-${time}` : `${folder}-${schedule}-${time}`;
  return raw
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, "-")
    .replace(/-{2,}/g, "-")
...
export function BatchDocumentJobsPanel({ locale }: BatchDocumentJobsPanelProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError("");
    try {
      const [nextJobs, nextRuns] = await Promise.all([listBatchDocumentJobs(), listBatchDocumentRuns(undefined, 30)]);
      setJobs(nextJobs);
      setRuns(nextRuns);
    } catch (loadError) {
  ...
  const jobColumns = useMemo<GridColDef<BatchDocumentJob>[]>(
    () => [
      { field: "name", headerName: t.columns.name, flex: 1, minWidth: 140 },
      { field: "directoryPath", headerName: t.columns.directory, flex: 1.2, minWidth: 180 },
      {
        field: "scheduleType",
      ...
      {
        field: "lastRunAt",
        headerName: t.columns.lastRun,
        flex: 1,
        minWidth: 150,
        valueGetter: (_value, row) => formatDateTime(row.lastRunAt, locale, "-")
      }
    ],
  ...
  const runColumns = useMemo<GridColDef<BatchDocumentRun>[]>(
    () => [
      { field: "jobName", headerName: t.columns.job, flex: 1, minWidth: 140 },
      {
        field: "triggerType",
  ...
  return (
    <article className="panel admin-card batch-documents-panel">
      ...
      <form className="settings-card batch-job-form" onSubmit={(event) => void handleSubmit(event)}>
        ...
        {form.directoryPath ? (
          <div className="batch-path-display">
            <span>{t.form.selectedDirectory}</span>
            <code>{form.directoryPath}</code>
          </div>
        ) : null}
        {form.directoryPath ? (
          <div className="batch-auto-name">
            <span>{t.form.autoName}</span>
            <strong>{autoJobName}</strong>
            <small>{t.form.autoNameHint}</small>
          </div>
        ) : null}
      ...
      {selectedRun ? (
        <section className="panel result-panel batch-run-detail">
          <div className="section-head">
            <div>
          ...
          <div className="case-stats">
            ...
            <div>
              <span>{t.runDetail.startedAt}</span>
              <strong>{formatDateTime(selectedRun.startedAt, locale, "-")}</strong>
            </div>
            <div>
              <span>{t.runDetail.finishedAt}</span>
              <strong>{formatDateTime(selectedRun.finishedAt, locale, "-")}</strong>
            </div>
          </div>
          ...
          <div className="feedback-datagrid-wrap">
            <DataGrid
              ...
              columns={[
                ...
                {
                  field: "extractedChars",
                  headerName: t.fileColumns.chars,
                  width: 110,
                  valueGetter: (_value, row) => row.extractedChars ?? "-"
                },
                {
                  field: "chunkCount",
